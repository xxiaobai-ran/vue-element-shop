  ...
  <div>
    ...
    <el-card>
      ...
      <el-tabs v-model="activeName" @tab-click="handleTabClick">
        <!--添加动态参数的面板-->
        <!--name为many和only是由后台的API决定的，本来自带的是first和second-->
        ...
        <el-tab-pane label="静态属性" name="only">
          ...
          <!--静态参数表格-->
          <el-table :data="onTableData" border stripe>
            <!--展开行-->
            <el-table-column type="expand">
            </el-table-column>
            <!--索引列-->
    ...
    <el-dialog
            ...
            @close="editParams"
    >
      <el-form :model="editForm" :rules="addFormRules" ref="changeFormRef" label-width="100px" class="demo-ruleForm"
...
<script>
  export default {
    ...
    methods: {
      ...
      //获取参数的列表数据
      async getParamsData() {
        //获取参数的列表数据---无论是级联选择器还是动态参数，静态属性，都是要获取数据信息的,所以在handleChange和handleTabClick中调用该函数
        //证明选中的不是三级分类
        ...
        if (res.meta.status !== 200) {
          ...
          //获取attr_vals，渲染成一个一个小标签
          res.data.forEach(item => {
            //要先对这个数组做判断，不然渲染出来的空数组也有图标
      ...
      async showEditDialog(attr_id) {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error('获取当前参数查询失败')
        } else {
      ...
      async handleInputConfirm(row) {
        ...
        //没有return,证明输入了内容，需要展示出来，并添加到数据库中
      ...
      showInput(row) {
        ...
        //才会指定回调函数中的代码
        //前面布尔值设为true了之后，没有被渲染出来，还是一个按钮，不是文本框，直接用focus获取焦点，获取不到
        this.$nextTick(_ => {
